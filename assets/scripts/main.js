...
function SlowVideo(x) {
  var videos = document.getElementsByClassName(x + '-video');
  for (var i = 0; i < videos.length; i++) {
    videos[i].playbackRate = videos[i].playbackRate * 0.5;
    // videos[i].play();
  }

  var msg = document.getElementById(x + '-msg');
  msg.innerHTML = 'Speed: ' + '×' + (videos[0].playbackRate * 2).toFixed(2);
...
function RestartVideo(x) {
  ...
  for (var i = 0; i < videos.length; i++) {
    videos[i].pause();
    // videos[i].playbackRate = 0.5; // Set to half speed
    videos[i].currentTime = 0;
    videos[i].play();
  }

  // Set button to pause icon since video is now playing
  if (toggleIcon) {
